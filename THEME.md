...
- Generic neutral placeholder product image (`assets/placeholder-product-image.png`) — for products with no real photo yet, referenced by the draft catalog import; see `docs/catalog-import.md` (Photos section) for how it gets a real CDN URL before import.
- Real shipping-rate + authenticity messaging live in the theme config itself (no Admin setup needed for these two): the announcement bar (`sections/header-group.json`) rotates the confirmed delivery rates (1.5 KWD Kuwait / 5.5 KWD rest of GCC) and an authenticity line; the homepage (`templates/index.json`) has a trust-badges section (Dawn's native Multicolumn) pre-filled with authenticity, delivery, and MyFatoorah-backed payment claims. Both bilingual AR/EN in one string, since the store has no second language configured yet (see below).
